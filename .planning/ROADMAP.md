...
**Plans**: 3 plans
  - [ ] 05-01-PLAN.md — Schemas + plumbing + Wave 0 verification (sessions.ts/sets.ts Zod, 5 new setMutationDefaults in client.ts, persister throttleTime:500 + AppState background-flush, 3 new tsx scripts + 2 extended for 25-set FIFO replay)
  - [ ] 05-02-PLAN.md — Workout vertical slice (sessions/sets/last-value resource hooks + workout/[sessionId] screen + Starta pass CTA on plans/[id] + F7 chip + Avsluta-overlay; F5+F6+F7+F8 closed end-to-end)
  - [ ] 05-03-PLAN.md — ActiveSessionBanner + draft-session-recovery overlay on (tabs)/index + Passet sparat toast + test-rls.ts cross-user extension + F13 manual brutal-test recipe + human-verify gate (closes ROADMAP success #5 + #6)
**UI hint**: yes
