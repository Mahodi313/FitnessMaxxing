...
**Requirements**: F1
**Success Criteria** (what must be TRUE):
  1. Användare kan registrera nytt konto med email + lösen från `(auth)/sign-up.tsx`. **Email-bekräftelse krävs** i nuvarande Supabase-konfiguration (Studio-toggle ON; config.toml `enable_confirmations=false` ej pushad pga localhost-värden i `site_url`/redirects). Användaren får bekräftelsemail, klickar länken (öppnas i webbläsare i V1), och loggar sedan in från `(auth)/sign-in.tsx` → hamnar i `(app)`-gruppen. **Deferred till V1.1 (Phase 8):** deep-link-handler för bekräftelselänken så att flödet stannar i appen. Se `.planning/phases/03-auth-persistent-session/03-UAT.md` Gaps-sektion för full kontext.
  2. Användare kan logga in från `(auth)/sign-in.tsx`; fel-validering via Zod visar fältfel inline (RHF + Zod 4) **vid submit** (mode ändrades från `onBlur` till submit-only under verification — se 03-UAT.md Test 3 note).
...
**Plans**: 4 plans
  - [x] 06-01a-PLAN.md — [Wave 1] Database + Wave 0 fixtures (Migration 0006 deploys THREE Postgres RPC functions: get_session_summaries (F9), get_exercise_chart (F10 chart), get_exercise_top_sets (F10 Senaste 10 passen with reps + session_id per source-session — BLOCKER-2 fix); SECURITY INVOKER + set search_path = ''; gen:types regenerates database.ts; verify-deploy.ts asserts all three functions; NEW test-exercise-chart.ts harness with 13 assertions (5 F9 + 5 F10 chart + 3 F10 top-sets) green against deployed Supabase; test-rls.ts Phase 6 extension adds 5 cross-user assertions including get_exercise_top_sets RLS + FK cascade; manual UAT markdown scaffolded for later plans)
  - [x] 06-01b-PLAN.md — [Wave 2] F9 client-tier vertical slice (depends_on 06-01a) — sessions.ts adds useSessionsListInfiniteQuery with SessionSummary Zod schema; keys.ts adds sessionsKeys.listInfinite; client.ts existing ['session','finish'] onSettled adds sessionsKeys.listInfinite invalidation per A8 executor-trap; (tabs)/history.tsx becomes cursor-paginated FlatList with pull-to-refresh + empty-state + Display heading Historik
  - [x] 06-02-PLAN.md — [Wave 3] F9 session-detail + delete vertical slice (depends_on 06-01b) — sessions.ts adds useDeleteSession(sessionId?) scope-bound hook; client.ts adds block 14 ['session','delete'] setMutationDefaults handling InfiniteQuery { pages, pageParams } envelope per Pitfall 6; new /history/[sessionId].tsx route — summary-header chips + card-per-exercise read-only with sets inline + Stack header right ellipsis-horizontal trigger + inline-overlay overflow menu + inline-overlay delete-confirm via Phase 4 commit e07029a + Reanimated toast "Passet borttaget" + useFocusEffect overlay reset per Pitfall 7; manual UAT markdown extended with delete-pass online + offline + freezeOnBlur sections
  - [x] 06-03-PLAN.md — [Wave 4] F10 per-exercise chart slice (depends_on 06-01a + 06-01b + 06-02) — keys.ts adds exerciseChartKeys + exerciseTopSetsKeys factories; new lib/queries/exercise-chart.ts with TWO hooks (useExerciseChartQuery + useExerciseTopSetsQuery) + Zod schemas (ChartRowSchema + TopSetRowSchema with reps int constraint) + windowToSince helper; new components/segmented-control.tsx NativeWind-baserat reusable; new /exercise/[exerciseId]/chart.tsx route — MetricToggle + WindowToggle (default 3M) + memoized CartesianChart with dep array EXACTLY [chartQuery.data] per D-21 + FULL Skia tooltip (RoundedRect + two Text lines via useDerivedValue per UI-SPEC §ChartPressCallout — BLOCKER-1) + TWO-STATE empty-state (window-empty vs all-time-empty via second useExerciseChartQuery for 'All' window — BLOCKER-3) + tappable Senaste 10 passen rows with `${weight} kg × ${reps}` routing to /history/<session_id> via the new top-sets RPC (BLOCKER-2); plans/[id].tsx PlanExerciseRow inserts chart-icon Pressable BETWEEN edit and remove with hitSlop={6} per UI-SPEC line 286 (D-24, WARN-5/6 fixes); manual UAT markdown extended with 7 chart sections
**UI hint**: yes
...
**Plans**: 5 plans
  - [x] 07-01-PLAN.md — Theme toggle (F15) — useColorScheme migration (10 files atomic), Tema-sektion in Inställningar with SegmentedControl + AsyncStorage, ThemeBootstrap in root layout + dynamic StatusBar
  - [x] 07-02-PLAN.md — RPE inline input + history-suffix (F11) — setFormSchema.rpe schema-stretch (preprocess + bounds), third Controller in workout inline-row + Klart-button shrink, conditional · RPE suffix on history set-rows
  - [x] 07-03-PLAN.md — Notes capture in AvslutaOverlay (F12) — widen useFinishSession payload + amend setMutationDefaults block 10 with optimistic notes cache write; AvslutaOverlay TextInput + KeyboardAvoidingView + counter + handleConfirm payload
  - [x] 07-04-PLAN.md — Notes view + edit in history-detail (F12) — new useUpdateSessionNotes hook + 15th setMutationDefaults block; notes-block above SummaryHeader with two-mode affordance + inline-overlay edit pattern + useFocusEffect cleanup
  - [x] 07-05-PLAN.md — Core-flow ≤2 min UAT + Phase closeout — author 07-HUMAN-UAT.md + run pre-flight code gates + checkpoint:human-verify for manual iPhone UAT on all 19 SPEC acceptance rows + 3 timed core-flow runs
**UI hint**: yes
...
**Requirements (V1.1)**: F14 (Apple Sign-In), F15-toggle (om inte i V1 Phase 7), F17-UI (set-typ-toggling), F18 (PR-detection), F19 (vilo-timer), **F1.1 Email-confirmation deep-link handler** (carry-over från Phase 3 — accepted-deferred 2026-05-09; bekräftelselänk öppnas i webbläsare i V1, ska öppna appen direkt i V1.1; se `.planning/phases/03-auth-persistent-session/03-UAT.md` gaps)
**Research flag**: F19 vilo-timer (`expo-notifications` + `expo-keep-awake`, JS-suspension-trap); F1.1 deep-link (Expo `Linking` API + Supabase `verifyOtp`/`exchangeCodeForSession`, Universal Links vs custom scheme `fitnessmaxxing://`)
**Plans**: TBD
...
| 7. V1 Polish Cut | 5/5 | Complete   | 2026-05-16 |
