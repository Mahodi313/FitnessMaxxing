...
| PR-04 | Session-level `get_session_pr_flags(p_session_ids uuid[])` cross-user isolation + `has_pr` correctness (boolean only — `bool_or` over the same window engine; no SQL display e1RM, D-08) | T-13-01 | Caller-JWT-scoped rows; `has_pr` per session | integration | `npm run test:rls` | ✅ extend `scripts/test-rls.ts` | ⬜ pending |
...
- [ ] `app/scripts/test-rls.ts` — extend with cross-user assertions for all four new RPCs (`get_exercise_pr_history`, `get_best_working_sets`, `get_exercise_sets_in_range`, `get_session_pr_flags`) (CLAUDE.md "cross-user verification is a gate"); assert `was_pr` + `has_pr` correctness on a seeded progression fixture
- [ ] `app/scripts/verify-deploy.ts` — add `phase13Functions` array (all four new RPC names) to the INVOKER + `search_path` `pg_proc` check (mirror existing `verify-deploy.ts` pattern)
...
- [x] All tasks have `<automated>` verify or Wave 0 dependencies (or are listed Manual-Only)
- [x] Sampling continuity: no 3 consecutive tasks without automated verify
- [x] Wave 0 covers all MISSING references (`test-e1rm.ts`, `test-rls.ts` extension for 4 RPCs, `verify-deploy.ts` array, `gen:types`)
- [x] No watch-mode flags
- [x] Feedback latency < 60s
- [x] `nyquist_compliant: true` set in frontmatter

**Approval:** approved 2026-06-14 (strategy design; `wave_0_complete` flips true during execute-phase Wave 0)
