...
### Resolved open questions (post-research, 2026-06-09)
- **D-11 (OQ-1):** Install **`react-native-svg`** (Software Mansion, via `npx expo install react-native-svg`, SDK-54 pin ~15.x) for `Icon`/`Logo`/`AppIcon` — 1:1 `<Path d>` port of the lib.jsx icon strings. This is the one approved new dependency this phase; DSGN-05's "no new charting dep" applies to charting only, and `ProgressRing`/`Sparkline` still use the already-installed Skia. NativeWind/Tailwind/Skia pins stay untouched.
- **D-12 (OQ-7):** `0007_profiles_weekly_goal.sql` is **NOT in Phase 8 scope.** Phase 8 stays **zero-schema-change** (matches CONTEXT/REQUIREMENTS/UI-SPEC); the migration moves to the later dashboard phase that consumes `weekly_goal`. No schema-push / type-gen / RLS-test task in this phase.
- **OQ-2/3/4/5/6 → follow RESEARCH.md recommendations:** component dir = flat `app/components/ui/`; light-mode rgba → solid-hex `forge.text2/text3/border/borderStrong/accentSoft` tokens with `dark:` opacity overrides; expose 3 Inter Display weights as 3 fontFamily classes (`font-display` 400 / `font-display-semibold` 600 / `font-display-bold` 700); `TabBar` is **build-only** (gallery), wired live in Phase 9+; gallery route `app/(app)/_forge-gallery.tsx` stays out of the tab bar, reachable via `router.push` in dev, `__DEV__`-guarded against release.
